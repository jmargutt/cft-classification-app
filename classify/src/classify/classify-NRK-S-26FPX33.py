...
def download_blob(container, blob_directory):
    ...
    for blob in blob_files:
        if blob_directory in f"{blob.name}":
            blob_client = get_blob_service_client(f"{blob.name}", container)
...
def inference(data, model_path, target_col, text_col="text"):
    ...
    # print(f"predictions {len(predictions)}, data {len(data)}")
...
def main(email, ds_id, model, overwrite):
    ...
    print(f"got {len(data)} messages to analyse")
    ...
    # model_path = r"C:\Users\JMargutti\OneDrive - Rode Kruis\Rode Kruis\digital-cea\few-shot-classification-api\models\COVID-19 (English)"
    ...
    for category in data['category'].dropna().unique():
        ...
        model_code_path = os.path.join(model_path, 'classify_code', category)
    ...
    for ix, row in data.iterrows():
        ...
        if result != "success":
            print(f"message {row['id']} not found")
